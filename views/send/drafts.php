...
<script type="text/javascript">
	...
	function deleteDraftMail(send_mail_id) {
...
<div class="container">
    <h1 class="my-4"><?php esc_html_e( 'Drafts', 'bluff-post' ) ?></h1>
    <hr class="my-4">
	<?php if ( ! empty( $errors ) ) : ?>
		...
		<div class="col-sm-12">
			...
			<div class="card">
				<div class="card-body">
					<?php if ( 0 < count( $mails ) ) : ?>
						<table class="table">
							...
							<tr>
								...
								<th><?php esc_html_e( 'Update', 'bluff-post' ) ?></th>
								<th></th>
							</tr>
							</thead>
							<tbody>
							<?php foreach ( $mails as $mail ) : ?>
								...
								<tr>
									<td class="text-overflow-ellipsis">
										<a href="<?php echo esc_url( admin_url( 'admin.php?page=blfpst-send-mail&admin_action=edit_draft&send_mail_id=' . $send_mail_id ) ) ?>"><?php echo esc_html( $subject ) ?></a>
									</td>
									<td>
									...
									<td>
										<small><?php echo esc_html( blfpst_localize_datetime_string( $mail->updated_at ) ) ?></small>
									</td>
									<td>
                                        <a href="#" role="button" onclick="deleteDraftMail(<?php echo esc_html( $send_mail_id ) ?>)"><i class="bi bi-x-circle-fill"></i></a>
									</td>
								</tr>
							<?php endforeach ?>
							</tbody>
					...
					<?php else : ?>
						<?php esc_html_e( 'There is no mail in the draft.', 'bluff-post' ) ?>
					<?php endif ?>
				</div>
			</div>
			<?php if ( $total_page > 1 ) : ?>
				<nav class="mt-4">
					<ul class="pagination">
						...
						?>
						<li class="page-item">
							<a href="<?php echo esc_url( admin_url( 'admin.php?page=blfpst-send-mail-drafts&page_num=' . $pre_page ) ) ?>"
						...
						for ( $i = $start_page; $i < $stop_page; $i ++ ) {
							...
							<li class="page-item<?php if ( $page_num == $i ) { ?> active<?php } ?>">
								<a href="<?php echo esc_url( admin_url( 'admin.php?page=blfpst-send-mail-drafts&page_num=' . $i ) ) ?>"
                                   class="page-link"><?php echo( $i + 1 ) ?></a>
							</li>
							<?php
						}
						?>
						<li class="page-item">
							<a href="<?php echo esc_url( admin_url( 'admin.php?page=blfpst-send-mail-drafts&page_num=' . $next_page ) ) ?>"
...
<form method="post" style="display: inline">
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
					<p><?php esc_html_e( 'Are you sure you want to delete this draft?', 'bluff-post' ) ?></p>
				</div>
	...
	<!-- /.modal -->
	<input type="hidden" name="admin_action" value="delete"> <input type="hidden" name="send_mail_id" value="0">
