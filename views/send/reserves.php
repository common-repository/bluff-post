...
?>
<div class="container">
    <h1 class="my-4"><?php esc_html_e( 'Reservation list', 'bluff-post' ) ?></h1>
    <hr class="my-4">
	<?php if ( ! empty( $errors ) ) : ?>
		...
		<div class="col-sm-12">
			<div class="card">
				<div class="card-body">
					<?php if ( 0 < count( $mails ) ) : ?>
						<table class="table">
							...
							<tr>
								...
								<th><?php esc_html_e( 'Reservation date', 'bluff-post' ) ?></th>
							</tr>
							</thead>
							<tbody>
							<?php foreach ( $mails as $mail ) : ?>
								...
								<tr>
									<td class="text-overflow-ellipsis">
										<a href="<?php echo esc_url( admin_url( 'admin.php?page=blfpst-send-mail-reserves&admin_action=info&send_mail_id=' . $mail->id ) ) ?>"><?php echo esc_html( $subject ) ?></a>
									</td>
									<td>
									...
									<td>
										<small><?php echo esc_html( blfpst_localize_datetime_string( $mail->reserved_at ) ) ?></small>
									</td>
								</tr>
							<?php endforeach ?>
							</tbody>
					...
					<?php else : ?>
						<?php esc_html_e( 'There is no e-mail that has been reserved.', 'bluff-post' ) ?>
					<?php endif ?>
				</div>
			</div>
			<?php if ( 1 < $total_page ) : ?>
				<nav class="mt-4">
					<ul class="pagination">
						...
						?>
						<li class="page-item">
							<a href="<?php echo esc_url( admin_url( 'admin.php?page=blfpst-send-mail-reserves&page_num=' . $pre_page ) ) ?>"
						...
						for ( $i = $start_page; $i < $stop_page; $i ++ ) {
                            ...
                            <li class="page-item<?php if ( $page_num == $i ) { ?> active<?php } ?>">
								<a href="<?php echo esc_url( admin_url( 'admin.php?page=blfpst-send-mail-reserves&page_num=' . $i ) ) ?>" class="page-link"><?php echo( $i + 1 ) ?></a>
							</li>
							<?php
						}
						?>
						<li class="page-item">
							<a href="<?php echo esc_url( admin_url( 'admin.php?page=blfpst-send-mail-reserves&page_num=' . $next_page ) ) ?>"
