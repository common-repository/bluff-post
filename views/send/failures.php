...
?>
<div class="container">
    <h1 class="my-4"><?php esc_html_e( 'Failure', 'bluff-post' ) ?></h1>
    <hr class="my-4">
	<?php if ( ! empty( $errors ) ) : ?>
					...
					<ul class="pagination">
						...
						for ( $i = $start_page; $i < $stop_page; $i ++ ) {
							...
							<li class="page-item<?php if ( $page_num == $i ) { ?> active<?php } ?>">
								<a href="<?php echo esc_url( admin_url( 'admin.php?page=blfpst-send-mail-failures&page_num=' . $i ) ) ?>" class="page-link"><?php echo( $i + 1 ) ?></a>
							</li>
							<?php
						}
						?>
